...
void updateall()
{
	...
	for (p = head; p != NULL; p = p->next) {
		p->job->wait_time += 1;
		if (p->job->curpri < 3) p->job->curpri += 1;
	}
}
	...
	if (current && current->job->jid == deqid) {
		...
		if (head) {
			...
			*/
			if (select) {
				selectprev->next = select->next;
				if (select == selectprev) head = head->next;
				select->next=NULL;
			}
		}
...
void do_stat()
{
	...
	*/
	...
	printf( "JID\tPID\tNAME\tOWNER\tRUNTIME\tWAITTIME\tDEFPRI\tCURPRI\tSTATE\tCREATTIME\n");
